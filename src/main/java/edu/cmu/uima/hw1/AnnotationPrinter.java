...
public class AnnotationPrinter extends CasConsumer_ImplBase implements CasObjectProcessor {
  ...
  public synchronized void processCas(CAS aCAS) throws ResourceProcessException {
    ...
    Iterator<Annotation> it = jcas.getAnnotationIndex(GeneMention.type).iterator();
    while (it.hasNext()) {
      GeneMention mention = (GeneMention) it.next();
      try {
        fileWriter.write(mention.getSentenceIdentifier() + "|" + mention.getStartOffset() + " "
                + mention.getEndOffset() + "|" + mention.getPhrase() + "\n");
      } catch (IOException e) {
        e.printStackTrace();
      }
    }
